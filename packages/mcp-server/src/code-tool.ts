...
const prompt = `Runs JavaScript code to interact with the API Dental Pro API.
...
const remoteStainlessHandler = async (
  ...
  const localClientEnvs = { API_DENTAL_API_KEY: requireValue(readEnv('API_DENTAL_API_KEY') ?? client.apiKey, 'set API_DENTAL_API_KEY environment variable or provide apiKey client option'), API_DENTAL_PRO_BASE_URL: readEnv('API_DENTAL_PRO_BASE_URL') ?? client.baseURL ?? undefined };
  ...
  const res = await fetch(codeModeEndpoint, {
    ...
    body: JSON.stringify({
      ...
      code,
      intent,
      client_opts: { sdkSource: readEnv('API_DENTAL_PRO_SDK_SOURCE'), sdkLang: readEnv('API_DENTAL_PRO_SDK_LANG') },
  ...
  try {
    const resp = await new Promise<Response>((resolve, reject) => {
      ...
      const opts = {
          ...
          ...(client.sdkSource != null ? { sdkSource: client.sdkSource } : undefined),
          ...(client.sdkLang != null ? { sdkLang: client.sdkLang } : undefined),
          defaultHeaders: {
