...
import { ClearCoverage, ClearCoverageRequestParams, ClearCoverageRequestResponse } from './resources/clear-coverage';
...
export interface ClientOptions {
   ...
   * Set the log level.
   *
   * Defaults to process.env['API_DENTAL_PRO_LOG'] or 'warn' if it isn't set.
   */
...
export class APIDentalPro {
  ...
  constructor({
    ...
    this.timeout = options.timeout ?? APIDentalPro.DEFAULT_TIMEOUT /* 1 minute */;
    this.logger = options.logger ?? console;
    const defaultLogLevel = 'warn';
    // Set default logLevel early so that we can log a warning in parseLogLevel.
    this.logLevel = defaultLogLevel;
    this.logLevel = parseLogLevel(options.logLevel, 'ClientOptions.logLevel', this) ?? parseLogLevel(readEnv('API_DENTAL_PRO_LOG'), 'process.env[\'API_DENTAL_PRO_LOG\']', this) ?? defaultLogLevel;
  ...
  static APIDentalPro = this;
  static DEFAULT_TIMEOUT = 60000 // 1 minute

  static APIDentalProError = Errors.APIDentalProError;
