...
describe('logging', () => {
  ...
  beforeEach(() => {
    process.env = { ...env };
    process.env['API_DENTAL_PRO_LOG'] = undefined;
  });
  ...
  const forceAPIResponseForClient = async (client: APIDentalPro) => {
  ...
  test('warn when env var level is invalid', async () => {
    ...
    const client = new APIDentalPro({ logger: logger, apiKey: 'My API Key' });
    expect(client.logLevel).toBe('warn');
    expect(warnMock).toHaveBeenCalledWith('process.env[\'API_DENTAL_PRO_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]');
  });
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['API_DENTAL_PRO_BASE_URL'] = undefined;
    });
...
describe('idempotency', () => {
  test.skip('key can be set per-request', async () => {
    const client = new APIDentalPro({ baseURL: process.env["TEST_API_BASE_URL"] ?? 'http://127.0.0.1:4010', apiKey: 'My API Key' });
